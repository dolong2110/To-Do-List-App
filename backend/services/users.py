...
from backend.models import model as _models
...
async def create_user(user: _schemas.UserCreate, db: _orm.Session):
    user_obj = _models.User(
        email = user.email, hashed_password=_pwd.hash(user.hashed_password)
    )
    db.add(user_obj)
    db.commit()
    db.refresh(user_obj)
...
async def create_access_token(user: _models.User, expires_delta: Optional[_dt.timedelta]=None):
    user_obj = _schemas.User.from_orm(user)
    pay_load = user_obj.dict()

    if expires_delta:
        expire = _dt.utcnow() + expires_delta
    else:
        expire = _dt.utcnow() + _dt.timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES)

    pay_load.update({"exp": expire})
    token = _jwt.encode(pay_load, JWT_SECRET, algorithm="HS256")
